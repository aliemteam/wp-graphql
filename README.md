...
`addMedia(file: String!, filename: String!)` | Create a new media item.<br>**Note:** `file` must be of type `Blob`, `File`, or `ArrayBuffer`.
...
`addPost(title: String!, content: String!, excerpt: String!)` | Create a new post.<br>**Note:** Only one of `title`, `content`, or `excerpt` is required for a sucessful request.
